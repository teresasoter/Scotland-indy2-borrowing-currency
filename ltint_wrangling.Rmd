...
df_merged_2006_2020 <- df_merged_2006_2020 %>% 
  ...
  mutate_ntile(col = debt_gdp, n= 3,
  new.col = third_debt_euro_2019)
```
mutate_ntile(
  DT,
  col,
...
Visualising relationship between debt-to-GDP ratio and long term interest rates with size of bubbles representing GDP (not logged)
```{r}
p9 <- df_merged_2006_2020 %>% 
    filter(TIME==2019, LOCATION %in% eurozone) %>% 
    ggplot(aes(x = LTINT, y = debt_gdp, size = GDP_MLN_USD, colour=LOCATION)) + 
    geom_point(alpha=0.7)
ggplotly(p9) 
```    
...
```{r}
write_csv(df_deficit_pct,"data_output/ltint_dataset_complete.csv")
