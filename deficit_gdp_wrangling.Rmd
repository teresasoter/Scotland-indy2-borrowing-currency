...
df_deficit_pct <- df_deficit_pct %>% #this function uses longform
  group_by(LOCATION) %>% 
  filter(TIME != 2009 | TIME != 2010 | TIME != 2011 | TIME != 2012 | TIME != 2013) %>%
...
Creating maximum values of 5-rolling minimums without GFC years but using rolling minimum levels that included GFC years when calculated
```{r}
max_min_5_no_GFC <- df_deficit_pct %>% 
  group_by(LOCATION) %>%
  filter(TIME != 2009 | TIME != 2010 | TIME != 2011 | TIME != 2012 | TIME != 2013) %>% 
  summarise(max_5_GFC = max(min_05y, na.rm=TRUE))
```

Creating maximum values of 5-rolling minimums without GFC years using rolling minimum levels that did not take the GFC years into account when calculated
```{r}
max_min_5_no_GFC_2 <- df_deficit_pct %>% 
  group_by(LOCATION) %>%
  summarise(max_5_GFC_2 = max(min_05y_no_GFC, na.rm=TRUE))
```
## Exploring data
...
The highest minimum deficit including the financial crisis was Greece -5.65
```{r}
    p9 <- max_min_5 %>%
...
### Plot 10
Visualising maximum minimum deficits over 5 years *excluding GFC years* (haven't figured out a way to keep the year information)

The rolling minimum was calculated *including* the GFC years
```{r}
    p10 <- max_min_5_no_GFC %>%
    ggplot(aes(LOCATION,max_5_GFC)) + 
    geom_col()
    ggplotly(p10)
```

### Plot 11
Visualising maximum minimum deficits over 5 years excluding GFC (haven't figured out a way to keep the year information)

The rolling minimum was calculated *excluding* the GFC years
```{r}
    p11 <- max_min_5_no_GFC_2 %>%
    ggplot(aes(LOCATION,max_5_GFC_2)) + 
    geom_col()
    ggplotly(p11)
```